...
def plot_gauge(probability, cutoff):
    """
    Affiche une jauge de probabilité de défaut.
    Args:
    probability (float): Probabilité de défaut prédite.
    cutoff (float): Valeur du cutoff pour décider d'un défaut.
    """
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            'threshold': {
                ...
                'thickness': 0.75,
                'value': cutoff * 100  # Position du cutoff sur la jauge
            }
        }
    ))
...
if uploaded_file:
    ...
    if st.button("Faire la Prédiction"):
        try:
            ...
            st.write("Résultats des prédictions (0 = Pas de défaut, 1 = Défaut) :")
            st.write(predictions)

            # Ajouter les résultats au DataFrame pour un affichage combiné
            data['Prédiction'] = predictions
            data['Probabilité de Défaut'] = probabilities
            
            st.write("Tableau des données avec les prédictions :")
            st.write(data)
            
            # Affichage de la jauge de probabilité pour chaque prédiction
            cutoff = 0.081  # Valeur du cutoff pour la décision de défaut
            
            for i, prob in enumerate(probabilities):
                st.write(f"Client {i+1} :")
                st.plotly_chart(plot_gauge(prob, cutoff))
                st.write(f"Cutoff pour le défaut : {cutoff * 100}%")
